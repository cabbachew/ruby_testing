...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 12 and the random number is 5' do
      subject(:game_five) { described_class.new(5) }
      
      it 'returns 7' do
        previous_step = 12
        result = game_five.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 11' do
      subject(:game_eleven) { described_class.new(11) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_eleven.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 26' do
      subject(:game_twenty_six) { described_class.new(26) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_twenty_six.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 31' do
      subject(:game_thirty_one) { described_class.new(31) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_thirty_one.play
        expect(result).to eq(7)
      end
    end
  end
