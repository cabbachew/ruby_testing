...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      ...
      it 'returns 7' do
        guess = game_min5_max9.make_guess
    ...
    context 'when min is 8 and max is 9' do
      ...
      it 'returns 8' do
        guess = game_min8_max9.make_guess
    ...
    context 'when min is 0 and max is 3' do
      ...
      it 'returns 1' do
        guess = game_min0_max3.make_guess
        expect(guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_minmax3) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        guess = game_minmax3.make_guess
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      subject(:game) { described_class.new(0, 9, random_number, 6) }
      ...
      it 'is game over' do
        expect(game).to be_game_over # OR: expect(game.game_over?).to be_truthy
      end
    end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:guess_is_7) { described_class.new(5, 8, number_range, 7) }

      before do
        guess_is_7.update_range
      end

      it 'updates min to the same value as max' do
        expect(guess_is_7.min).to eq(guess_is_7.max)
      end

      it 'does not update max' do
        expect(guess_is_7.max).to eq(8)
      end
    end
  end
end
